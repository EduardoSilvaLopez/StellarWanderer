...
def draw_rocks(surface, w, h, environment, player):
    ...
    than straight-line distance (which was exaggerating the shrink for
    off-centre rocks).
    """
    view_h = int(h * CONSOLE_TOP)
    ...
    NEAR_CLIP = 0.5   # metres; skip rocks whose near face is essentially at the camera
    MAX_DEPTH = 1000  # metres; render distance

    # The cube's front-to-back thickness (world Z) sits at a slightly different
    # depth than its near face, and screen-Y is altitude / depth (both non-linear
    # in depth). At altitude, that gap gets magnified far more than the cube's
    # actual width/height, so a physically-exact depth reads as an elongated
    # brick rather than a cube. We keep true depth for near/far-face *culling*
    # and for the horizontal (x) trapezoid that makes each rock's perspective
    # genuinely distinct, but render the thickness at a fraction of the real
    # size so the vertical stretch stays proportionate. 1.0 = physically exact.
    ...
    for z0, rock, half in visible_rocks:
        ...
        # z (depth) is rendered at ROCK_DEPTH_SCALE of the true size, centred
        # on the same near-face position, to avoid the altitude-driven
        # over-stretch described above.
        x0, x1 = rock.x - half, rock.x + half
        y0, y1 = 0.0, rock.size
        render_depth = rock.size * ROCK_DEPTH_SCALE
        z1 = z0 + render_depth  # far face forward depth (z0 already relative to player)
        ...
        # Project the (up to) 8 distinct corners we need.
        corners = {}
        ...
        # A lateral face is visible only if the camera sits outside it, i.e.
        # on the side its outward normal points toward.
        ...
        # Skip degenerate/too-small cubes (front face nearly a point).
